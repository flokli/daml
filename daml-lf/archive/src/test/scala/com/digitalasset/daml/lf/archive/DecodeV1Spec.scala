...
class DecodeV1Spec
  ...
  "The keys of builtinFunctionMap correspond to Protobuf DamlLf1.BuiltinFunction" in {
    (DecodeV1.builtinInfos.map(_.protoName).toSet + DamlLf1.BuiltinFunction.UNRECOGNIZED) shouldBe
  ...
  private def moduleDecoder(minVersion: LanguageMinorVersion) =
    new DecodeV1(minVersion)
      .ModuleDecoder(Ref.PackageId.assertFromString("noPkgId"), ImmArray.empty.toSeq, dummyModule)

  private val preNumericMinVersions = Table(
    "minVersion",
    List(1, 4, 6).map(i => LanguageMinorVersion.Stable(i.toString)): _*
  )
  ...
  "decodeExpr" should {
    ...
    "transparently apply TNat(10) to Numeric builtin if minVersion =< 1.dev" in {
      ...
      import LanguageMajorVersion.V1.minorVersionOrdering

      val positiveTestCases = Table("builtinInfo", numericBuilttins: _*)
      val negativeTetsCases = Table("builtinInfo", nonNumericBuiltins: _*)

      forEvery(preNumericMinVersions) { minVersion =>
        val decoder = moduleDecoder(minVersion)

        forEvery(positiveTestCases) { builtinInfo =>
          if (minorVersionOrdering.gteq(minVersion, builtinInfo.minVersion))
    ...
    "translated TNumeric such as if minVersion > 1.6" in {
      ...
      val testCases = Table("builtinInfo", (numericBuilttins ++ nonNumericBuiltins): _*)
